...
static int __init OpenR2_init(void)
{
	...
	printk(KERN_INFO "%s: >> %s\n","LogicDisplay",__func__);
   
	// Try to dynamically allocate a major number for the device

	result = alloc_chrdev_region(&OpenR2_dev, 0, minorNumbers, "OpenR2-LogicDisplay");
	if( result < 0 )
	{
	...
	printk(KERN_INFO "%s:   major=%d %d\n","LogicDisplay",MAJOR(OpenR2_dev),0);
	printk(KERN_INFO "%s:   major=%d %d\n","LogicDisplay",MAJOR(OpenR2_dev),1);
	printk(KERN_INFO "%s:   major=%d %d\n","LogicDisplay",MAJOR(OpenR2_dev),3);

	// udev virtual device class

        OpenR2_class = class_create(THIS_MODULE,"OpenR2-LogicDisplay");
	...
	{
		printk(KERN_WARNING "%s: %d %d\n","LogicDisplay",majorNumber,minor);

		OpenR2_devices[minor]=device_create(OpenR2_class,NULL,MKDEV(majorNumber,minor),NULL,"OpenR2-LogicDisplay%d",minor);
		if(IS_ERR(OpenR2_devices[minor]))
		{
			printk(KERN_WARNING "%s: device_create %d %d FAILED\n","LogicDisplay%",majorNumber,0);
